        ...
        private void Render()
        {
            ...
            for (int id = 0; id < plants.Count; id++)
            {
                ...
                if (!cbLarge.Checked && plants[id]["size"].ToString() == "l")
                    continue;
                if (!cbCheap.Checked && float.Parse(plants[id]["price"].ToString()) < 30)
                    continue;
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow plant in plants)
            {
                ...
                if (!cbLow.Checked && Int32.Parse(plant["star"].ToString()) <= 2)
                    continue;
                if (!cbMediumQuality.Checked && Int32.Parse(plant["star"].ToString()) >= 3 && Int32.Parse(plant["star"].ToString()) <= 4)
                    continue;
